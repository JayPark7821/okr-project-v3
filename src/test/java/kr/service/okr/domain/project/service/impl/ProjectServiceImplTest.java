package kr.service.okr.domain.project.service.impl;
...
class ProjectServiceImplTest {
	...
	void create_project() throws Exception {
		...
		ProjectInfo projectInfo = sut.registerProject(
			new ProjectSaveCommand("projectObjective", projectSdt, projectEdt, null), 999L, List.of());
	...
	void create_project_with_team_members() throws Exception {
		...
		ProjectInfo projectInfo = sut.registerProject(
			new ProjectSaveCommand("projectObjective", projectSdt, projectEdt, List.of("guest@email.com")), 999L,
			List.of(4L));
	...
	void deleteSingleProjectBy를_호출하면_참여중인_SINGLE_타입의_프로젝트는_모두_삭제된다() throws Exception {
		...
		sut.promoteNextProjectLeaderOrDeleteProject(3L);

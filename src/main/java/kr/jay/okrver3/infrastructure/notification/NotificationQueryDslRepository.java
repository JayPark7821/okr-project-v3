...
import static kr.jay.okrver3.domain.notification.QNotification.*;
	...
	public Page<Notification> findNotificationByUserSeq(Pageable pageable, Long userSeq) {
		List<Notification> results =
			queryFactory
				.select(notification)
				.from(notification)
				.where(notification.status.ne(kr.jay.okrver3.domain.notification.NotificationCheckType.DELETED)
